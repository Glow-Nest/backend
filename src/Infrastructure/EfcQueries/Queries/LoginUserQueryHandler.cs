...
public class LoginUserQueryHandler : IQueryHandler<LoginUserQuery, LoginUserResponse>
{
    ...
    public async Task<LoginUserResponse> HandleAsync(LoginUserQuery query)
    {
        if (query.Email == SalonOwnerEmail)
        {
            var salonOwner = await _context.Set<SalonOwner>()
                .Where(c => c.EmailAddress == query.Email) // LINQ query for salon owner
                .FirstOrDefaultAsync();
            ...
            {
                return Result<LoginUserResponse>.Fail(ClientErrorMessage.FailedToGenerateToken()).Data;
            }
            ...
            return Result<LoginUserResponse>.Success(response).Data;
        }
